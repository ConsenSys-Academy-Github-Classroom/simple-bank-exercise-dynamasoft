...
contract SimpleBank {
    ...
    /* Modifier */
    modifier requireEnrolled()
    {
      require(enrolled[msg.sender], "User is not enrolled");
      _;
    }

    modifier requireHasEnoughFund(uint withdrawAmount)
    {
      require(balances[msg.sender] >= withdrawAmount, "User does not have enough fund to withdraw");
      _;
    }
    ...
    payable returns (uint) {
      ...
      //** added the keyword payable at the top */
    
      // 2. Users should be enrolled before they can make deposits
      //** added the requireEnrolled modifier
    ...
    requireHasEnoughFund(withdrawAmount)
    public returns (uint) {
      ...
      //** added the requireHasEnoughFund modifier */
